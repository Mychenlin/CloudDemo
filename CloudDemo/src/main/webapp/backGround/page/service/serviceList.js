layui.config({
	...
	//加载页面数据
	var newsData = '';
	$.get("../../json/serviceList.json", function(data){
		var newArray = [];
		//单击首页“待审核文章”加载的信息
	...
	//添加服务类型
	$(".serviceAddLarge").click(function(){
		var index = layui.layer.open({
			title : "新增服务服务类型",
			type : 2,
			content : "serviceTypeAdd.html",
			area:['500px','450px'],
			shade:[0.8,'#393D49'],
		})
	})
	//修改服务类型
	$(".serviceUpdateLarge").click(function(){
		var index = layui.layer.open({
			title : "修改服务类型",
			type : 2,
			content : "serviceTypeUpdate.html",
			area:['500px','450px'],
			shade:[0.8,'#393D49'],
		})
	})
	...
	$("body").on("click",".serviceUpdateSmall",function(){  //编辑
		var index = layui.layer.open({
			title : "修改服务",
			type : 2,
			content : "serviceUpdate.html",
			area:['500px','450px'],
			shade:[0.8,'#393D49'],
		})
	})
		...
		function renderDate(data,curr){
			...
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
			    	...
			    	+'<td>'+currData[i].newsTime+'</td>'
			    	+'<td>'
					+  '<a class="layui-btn layui-btn-mini serviceUpdateSmall"><i class="iconfont icon-edit">&#xe629;</i> 编辑</a>'
					+  '<a class="layui-btn layui-btn-danger layui-btn-mini" data-id="'+data[i].newsId+'"><i class="layui-icon">&#xe640;</i> 删除</a>'
			        +'</td>'
			    	+'</tr>';
				}
			}else{
