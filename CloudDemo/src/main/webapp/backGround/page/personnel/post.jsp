  ...
  <head>
	...
	<script type="text/javascript" src="backGround/page/personnel/layui/layui.js"></script>
		...
		layui.use(['table','layer'],function(){
			...
			table.render({
				elem:'#tb',
				url:'position/seleposition',
				method:'post',
				page:true,
				limit:5,
				limits:[5,10,15],
				cols:[[
					{field:'pId',title:'职位编号'},
					{field:'pName',title:'职位名称',align:'center'},
					{field:'pBId',title:'部门名称',templet:'<div>{{d.pBId.bName}}</div>',align:'center'},
					{field:'pRemark',title:'职位备注',align:'center'},
			...
			$("#hui").click(function(){
				layer.open({
					type:2,
					title:'回收站',
					content:'backGround/page/personnel/huipost.jsp',
					area:['500px','500px'],
					shade:0.5,
					btn:'关闭',
					resize:false,  //不允许拉伸
			...
			$("#addBtn").click(function(){
				layer.open({
					type:2,
					title:'添加职位',
					content:'backGround/page/personnel/addpost.jsp',
					area:['450px','400px'],
					shade:0.5,
					btn:'关闭',
					resize:false,//不允许拉伸
			...
			table.on('tool(show)',function(obj){
				...
				//将一个JavaScript值(对象或者数组)转换为一个 JSON字符串
					...
					layer.open({
						type:2,
						title:'修改职位',
						content:'backGround/page/personnel/updatepost.jsp',
						area:['450px','400px'],
						shade:0.5,
						btn:'关闭',
						resize:false,   //不允许拉伸
				...
				if(obj.event=='delete'){
					layer.confirm("是否删除该数据?",function(){
						$.post("position/deletePosition",{id:data.pId},function(res){
							if(res>0){
								...
								layer.msg("没有找到编号为【"+data.pId+"】的数据",{time:2000});
							}else{
